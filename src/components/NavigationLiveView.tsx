...
import type { NavigationItem, LayoutProfiles, NavigationRoute } from '@/types/navigation';
...
export default function NavigationLiveView() {
  ...
  const [layoutProfiles, setLayoutProfiles] = useState<LayoutProfiles>({});
  const [routes, setRoutes] = useState<NavigationRoute[]>([]);
  const [expandedIds, setExpandedIds] = useState<Set<string>>(new Set());
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<'tree' | 'logs' | 'settings' | 'profiles' | 'routes'>('tree');
  ...
  async function loadLayoutProfiles() {
    try {
      const profiles = await navigationService.getLayoutProfiles();
  ...
  async function loadRoutes() {
    try {
      const routesData = await navigationService.getNavigationRoutes();
      ...
      <div key={item.id} className="select-none">
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            {item.view_type && (
              <span className="px-2 py-0.5 text-xs bg-blue-100 text-blue-700 rounded" title="View Type (v1.3.4)">
                {item.view_type}
              </span>
            )}
            {item.layout_profile && item.layout_profile !== 'backend_default' && (
              <span className="px-2 py-0.5 text-xs bg-green-100 text-green-700 rounded" title="Layout Profile (v1.3.4)">
                {item.layout_profile}
              </span>
            )}
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          {!loading && activeTab === 'profiles' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-gray-900">Layout Profiles (v1.3.4)</h2>
                <span className="px-3 py-1 bg-green-100 text-green-800 text-xs font-medium rounded-full">
                  {Object.keys(layoutProfiles).length} profile{Object.keys(layoutProfiles).length !== 1 ? 's' : ''}
                </span>
              </div>

              {Object.entries(layoutProfiles).map(([profileId, profile]) => (
                <div key={profileId} className="border border-gray-200 rounded-lg p-4 bg-white">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <Layout className="w-5 h-5 text-blue-600" />
                      <h3 className="font-semibold text-gray-900">{profile.label}</h3>
                    </div>
                    <code className="px-2 py-1 bg-gray-100 text-xs rounded">{profileId}</code>
                  </div>

                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      <h4 className="font-medium text-gray-700 mb-2">Zones</h4>
                      <div className="space-y-1">
                        <div className="flex justify-between">
                          <span>Header:</span>
                          <span className={profile.zones.header.visible ? 'text-green-600' : 'text-gray-400'}>
                            {profile.zones.header.visible ? 'Visible' : 'Hidden'}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>Sidebar:</span>
                          <span className={profile.zones.sidebar.visible ? 'text-green-600' : 'text-gray-400'}>
                            {profile.zones.sidebar.visible ? `Visible (${profile.zones.sidebar.width}px)` : 'Hidden'}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>Toolbar:</span>
                          <span className={profile.zones.toolbar.visible ? 'text-green-600' : 'text-gray-400'}>
                            {profile.zones.toolbar.visible ? 'Visible' : 'Hidden'}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>Footer:</span>
                          <span className={profile.zones.footer.visible ? 'text-green-600' : 'text-gray-400'}>
                            {profile.zones.footer.visible ? 'Visible' : 'Hidden'}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-1">
                        <div className="flex justify-between">
                          <span>Padding:</span>
                          <code className="text-xs bg-gray-100 px-2 py-0.5 rounded">{profile.options.content_padding}</code>
                        </div>
                        <div className="flex justify-between">
                          <span>Max Width:</span>
                          <code className="text-xs bg-gray-100 px-2 py-0.5 rounded">{profile.options.max_content_width}</code>
                        </div>
                        <div className="flex justify-between">
                          <span>Scroll:</span>
                          <code className="text-xs bg-gray-100 px-2 py-0.5 rounded">{profile.options.scroll_behavior}</code>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          {!loading && activeTab === 'routes' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-gray-900">Registered Routes (v1.3.4)</h2>
              ...
              {routes.length === 0 ? (
                ...
                <div className="space-y-2">
                  {routes.map(route => (
                    <div key={route.id} className="border border-gray-200 rounded-lg p-4 bg-white hover:border-blue-300 transition-colors">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            <Route className="w-4 h-4 text-purple-600" />
                            <code className="font-mono text-sm font-semibold text-gray-900">{route.route}</code>
                          </div>
                          <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm text-gray-600">
                            <div>
                              <span className="text-gray-500">Module:</span>
                              <code className="ml-2 px-2 py-0.5 bg-gray-100 rounded text-xs">{route.module_id}</code>
                            </div>
                            <div>
                              <span className="text-gray-500">View Type:</span>
                              <code className="ml-2 px-2 py-0.5 bg-blue-100 text-blue-800 rounded text-xs">{route.view_type}</code>
                            </div>
                            <div>
                              <span className="text-gray-500">Layout:</span>
                              <code className="ml-2 px-2 py-0.5 bg-green-100 text-green-800 rounded text-xs">{route.layout_profile}</code>
                            </div>
                            <div>
                              <span className="text-gray-500">Menu ID:</span>
                              <code className="ml-2 px-2 py-0.5 bg-gray-100 rounded text-xs">{route.menu_id || 'null'}</code>
                            </div>
                          </div>
                        </div>
                      </div>
