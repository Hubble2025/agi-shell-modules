/*
  ...
  1. **Performance Settings**
     ...
     - `max_tree_depth` - Maximum navigation hierarchy depth
     - `enable_advanced_indexes` - Toggle advanced DB tier

  2. **Security Settings**
     - `require_authentication` - Force authentication for all navigation
     - `enable_audit_logging` - Toggle audit trail
     - `max_failed_queries` - Rate limit threshold

  3. **Feature Flags**
     - `enable_live_updates` - Real-time subscriptions
     - `enable_soft_delete` - Soft delete instead of hard delete
     - `enable_versioning` - Navigation version control

  4. **API Settings**
     - `api_rate_limit` - Requests per minute
     - `max_batch_size` - Maximum bulk operations
     - `enable_public_api` - Allow unauthenticated access

  5. **UI Settings**
     - `default_icon` - Fallback icon for items
     - `theme` - UI theme configuration
     - `language` - Default language

  ## Schema Design

  - Key-value store with JSON validation
